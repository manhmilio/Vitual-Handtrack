...
try:
    ...
    raise RuntimeError(
        'Failed to import mediapipe. Install it in your active Python environment.'
    ) from exc

if not hasattr(mp, 'solutions'):
    raise RuntimeError(
        'This project requires the MediaPipe Solutions API (mp.solutions), but your installed '
        'mediapipe package does not provide it.\n'
        f'Current Python: {sys.executable}\n'
        'Fix: install the compatible version (mediapipe==0.10.21) from requirements.txt and run '
        'with that interpreter (Python 3.10/3.11 recommended).'
    )
            ...
            if results.multi_hand_landmarks!=None:
                ...
                for hand in results.multi_hand_landmarks:
                    ...
                    for landmark in hand.landmark:
                        singlehand.append((int(landmark.x*frame_w),int(landmark.y*frame_h)))
